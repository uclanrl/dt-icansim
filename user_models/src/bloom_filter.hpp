...
public:
    ...
    bloom_parameters(bloom_parameters const & o)
        :minimum_size(o.minimum_size),
          maximum_size(o.maximum_size),
          minimum_number_of_hashes(o.minimum_number_of_hashes),
          maximum_number_of_hashes(o.maximum_number_of_hashes),
          projected_element_count(o.projected_element_count),
          false_positive_probability(o.false_positive_probability),
          random_seed(o.random_seed){
        this->compute_optimal_parameters();  
    }

    virtual ~bloom_parameters()
    {
//        std::cout<<"~bloom_parameters"<<std::endl;
    }
...
public:
    ...
    //NOTE: when initializing a bloom_filter, do not use default constructor (since it does not generate salt)
    bloom_filter()
        : bit_table_(0),
          salt_count_(0),
          table_size_(0),
...
//         salt_ = f.salt_;
    ...
    //This is used to recover a bloom_filter with its "real" bit table content, such as sending along with a packet.
    bloom_filter(const bloom_filter& f, cell_type* btable)
        : bit_table_(0)
    {
/*        std::cout<<"make new bf"<<std::endl;
        salt_count_ = p.optimal_parameters.number_of_hashes;
        table_size_ = p.optimal_parameters.table_size;
        generate_unique_salt();
        raw_table_size_ = table_size_ / bits_per_char;
        ...
        bit_table_ = new cell_type[static_cast<std::size_t>(raw_table_size_)];
*/
        this->RebuildBitTable(f, btable);
    }

    bloom_filter& RebuildBitTable (const bloom_filter& f, cell_type* btable)
    {
        if (this != &f)
        {
            salt_count_ = f.salt_count_;
            ...
            if(bit_table_)
                delete[] bit_table_;
            bit_table_ = new cell_type[static_cast<std::size_t>(raw_table_size_)];
            std::copy(btable,btable + raw_table_size_,bit_table_);
            generate_unique_salt();
//         salt_ = f.salt_;
        }
        return *this;
    ...
    unsigned long long int GetRawTableSize()
    {
    ...
    void PrintBitTable()
    {
        for(int i=0; i<raw_table_size_; i++)
        {
            printf("%x ", bit_table_[i]);
        }
        printf("\n");
    }
    ...
    inline bool operator == (const bloom_filter& f) const
    {
        if (this != &f)
        {
            return
                ...
                (table_size_                         == f.table_size_)                         &&
                (raw_table_size_                     == f.raw_table_size_)                     &&
//                (projected_element_count_   == f.projected_element_count_)   &&                           //TODO: test this, these are commented because the operator &=...etc does not update element count
//                (inserted_element_count_             == f.inserted_element_count_)             &&
                (random_seed_                        == f.random_seed_)                        &&
                (desired_false_positive_probability_ == f.desired_false_positive_probability_) &&
                (salt_                               == f.salt_)                               &&
...
//         salt_ = f.salt_;
    ...
    {
//        std::cout<<"~bloom_filter"<<std::endl;
        delete[] bit_table_;
    }
